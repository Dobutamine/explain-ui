# P1 (cardiovascular paper) — verified reference list

*Companion working file for the Word/EndNote pass on `ExplainCircPaper(27012026)_WPdB_TA_WvM.docx`.
The `.docx` reference list (23 entries, EndNote-formatted, fully inline-cited [1]–[23], no orphans /
no dangling markers) was verified against PubMed on **2026-07-16**. All 20 journal entries resolve to
the correct article; PMIDs + DOIs are added below and the entries are rendered in the unified series
Vancouver style (matching P2–P7). Refs 10, 12 and 15 are a textbook, a 1965 dissertation and a 1920
pre-MEDLINE paper respectively — correctly not in PubMed. **This file is not injected into the `.docx`
automatically** (its EndNote field codes must be edited in Word); paste/reconcile from here during the
Word pass.*

## Actionable findings

1. **[8] Munneke — year should be 2021, not 2022.** PubMed indexes it as *Pediatr Res.* **2021**;91(1):116–128
   (PMID 33731808). The `.docx` says 2022 (the print-issue year); the rest of the series uses 2021. **Change
   2022 → 2021** for consistency.
2. **[19] Kluckow & Evans — the cited item is a 1-page letter/reply, not the seminal method paper.** The
   `.docx` cites *J Am Soc Echocardiogr.* 2014;27(7):**794** (PMID 24798866) — a single-page correspondence.
   If the intent is the *validated SVC-flow measurement*, the canonical primary source is **Kluckow M, Evans N.
   Superior vena cava flow in newborn infants: a novel marker of systemic blood flow. *Arch Dis Child Fetal
   Neonatal Ed.* 2000;82(3):F182–7 (PMID 10794790)**. Decide at the Word pass whether to swap or keep the letter.
3. **[18] Groves — 2011 vs 2010 is epub-vs-print only** (PMID 20971721; vol 96(2) is the March-2011 print
   issue, epub 2010). Keep 2011. No action needed beyond awareness.

## Series-name-collision clarifications (not errors — do NOT "reconcile" these away)

P1 legitimately cites *different* papers by authors who also appear elsewhere in the series with *other*
works. Keep them distinct at assembly:

- **[4] van Willigen 2022** = *Front Pediatr* review of fetal circulatory models (PMID 36210952) — **not**
  van Willigen **2026** *Pediatr Res* (the CHD-transition model cited in P3a/P5/P7).
- **[6] Sá-Couto 2006** = *Simul Healthc* neonatal-CV-pathophysiology simulator (PMID 19088566) — **not**
  Sá-Couto **2010** *Pediatr Res* (the transition simulator cited in P5/P7).
- **[9] May 2023** = *WIREs Mech Dis* modeling review (PMID 37002617) — **not** May **2025** *Matern Child
  Health* (Karger) digital-twins paper cited in P5/P7.
...
## Verified reference list (paste-ready, unified series format)
...
19. Kluckow M, Evans N. Superior vena cava flow is a clinically valid measurement in the preterm newborn. *J Am Soc Echocardiogr.* 2014;27(7):794. PMID 24798866. doi:10.1016/j.echo.2014.04.002. *(NB: 1-page letter/reply — see finding 2; consider swapping to the 2000 primary paper, PMID 10794790.)*
